...
model = load_model('custom_image_classifier.model')

# Define class names
class_names = ['camp', 'soldier', 'tank']
image_size = (224, 224)  # Ensure dimensions match the training image size

# Function to capture image from an IP webcam, compress it, and send it to the model
def capture_and_classify(model, class_names, output_file, ip_address):
    cap = cv.VideoCapture(ip_address)
    if not cap.isOpened():
    ...
    with open(output_file, 'w') as f:
        ...
        while True:
            ...
            # Display the frame
            cv.imshow('IP Webcam Feed', frame)
...
# IP address of the IP webcam stream
ip_address = 'http://172.20.10.2:8080/video'  # Replace with your IP webcam address

# Start capturing and classifying
capture_and_classify(model, class_names, 'predictions.txt', ip_address)
